...
const callbacks = {
  instagram: {
    setCookie: async (ctx: DinoMenuContext) => {
      ...
      await ctx.conversation.enter(
        dinoConversations.updateInstagramCookie.name,
      );
    },
  ...
  youtube: {
    setCookie: async (ctx: DinoMenuContext) => {
      ctx.menu.close();
      await ctx.conversation.enter(
        dinoConversations.updateYoutubeCookie.name,
      );
    },
    getCookie: async (ctx: DinoMenuContext) => {
      const cookie = await db.youtube.cookie.get() ?? "null";
      const { text, entities } = fmt([
        "YouTube cookie:\n\n",
        spoiler(cookie),
      ]);
